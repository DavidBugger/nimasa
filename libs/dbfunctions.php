...
*/
	...
	function getcheckdetails($user, $password)
	{
		...
		if ($numrows > 0) {
			...
			if ($row['user_disabled'] == '1') {
				...
				$label = "11";
			} else if ($pass_dateexpire != null && $expiration_date <= $today_date) {
				$label = "13";
				...
				$label = "1";
				$_SESSION['eagle_username_sess']  = $user;
				$_SESSION['eagle_role_id_sess']   = $row['role_id'];
				$_SESSION['eagle_firstname_sess'] = $row['firstname'];
				$_SESSION['eagle_lastname_sess']  = $row['lastname'];
				$_SESSION['eagle_super_agent_id'] = $row['super_agent_id'];
				$_SESSION['eagle_reg_status']     = $row['reg_status'];
				$_SESSION['eagle_approval']       = $row['user_approved'];
				$_SESSION['areaCode_sess']       	= $row['areaCode'];
				$_SESSION['transaction_id_sess']       	= $row['transaction_id'];
				$_SESSION['password'] = $row['password'];
				$_SESSION['street_under_allocated_area_sess']  = $row['street_under_allocated_area'];
				
				$_SESSION['eagle_password']       = $password;
				$_SESSION['eagle_last_page_load'] = time();
	...
	function marshalcheckDetails($user, $password,$device_serial_no)
	{	

	
		// var_dump($device_serial_no);exit; 
		//echo 'country code : '.$countrycode;
		$desencrypt = new DESEncryption();
		$key = $user; //"mantraa360";
		$cipher_password = $desencrypt->des($key, $password, 1, 0, null, null);
		...
		if ($numrows > 0) {
			...
			$getting_device_id = "SELECT * FROM devices where device_serial_no = '$device_serial_no'";
			$device_result = $this->db_query($getting_device_id);
			$device_id = $device_result[0]['device_serial_no'];
			$road_marshal_id = $device_result[0]['license_operator_id'];
			echo $marshal_verification = "SELECT * FROM userdata WHERE username = '$user'";
			$marshal_result = $this->db_query($marshal_verification);
			$device_license_operator = $marshal_result[0]['license_operator_id'];
			var_dump($road_marshal_id,$device_license_operator);exit;
		
			if($device_id !=$device_serial_no){
				$label = "16";
			} else if($road_marshal_id !=$device_license_operator){
				$label = "17";
			}else if ($row['user_disabled'] == '1') {
				$label = "2";
			} else if ($row['user_locked'] == '1') {
				$label = "3";
			} else if ($row['day_1'] == '0' && $ddate == '0') {
				...
				$_SESSION['eagle_approval']       = $row['user_approved'];
				$_SESSION['areaCode_sess']       	= $row['areaCode'];
				$_SESSION['license_operator_id'] = $row['license_operator_id'];
				$_SESSION['street_under_allocated_area_sess']  = $row['street_under_allocated_area'];
				$_SESSION['eagle_password']       = $password;
				$_SESSION['eagle_last_page_load'] = time();
				//            $login_resp = json_decode($dbobject->doApiCall(array("email"=>$_SESSION['username_sess'],"ip"=>$_SERVER['REMOTE_ADDR'],"location"=>"","password"=>$password,"platform"=>"web"),"login"), true );
	...
	public function CORS($data)
    {
        ...
        // $data = base64_encode(hash_file("sha256", file_get_contents($link), true));
